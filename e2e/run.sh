...
export KIND_CLUSTER_NAME=${KIND_CLUSTER_NAME:-carina-test-dev}
...
export REGISTRY=carina-controller
...
kind load docker-image --name="${KIND_CLUSTER_NAME}" --nodes=${KIND_WORKERS} ${REGISTRY}:${TAG}

echo "[dev-env] deploy carina"
helm repo add carina-csi-driver https://carina-io.github.io
helm search repo -l carina-csi-driver
helm install carina-csi-driver carina-csi-driver/carina-csi-driver --namespace kube-system --version v0.10.0 --set image.carina.tag=${TAG}
